...
public class MovimentoJ4 : MonoBehaviour
{
    Grid grid;
    ...
    public void OnMouseDown()
    {
        ...
        {
            //Anda para baixo
            if (grid.LocalToCell(transform.position).x == 4 && grid.LocalToCell(transform.position).y != -3)
            {
            ...
            else if (grid.LocalToCell(transform.position).y == -3 && grid.LocalToCell(transform.position).x != -6)
            {
