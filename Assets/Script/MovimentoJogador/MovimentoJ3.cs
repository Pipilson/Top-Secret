...
public class MovimentoJ3 : MonoBehaviour
{
    Grid grid;
    ...
    public void OnMouseDown()
    {
        ...
        {
            //Anda para baixo
            if (grid.LocalToCell(transform.position).x == 5 && grid.LocalToCell(transform.position).y != -2)
            {
            ...
            else if (grid.LocalToCell(transform.position).y == -2 && grid.LocalToCell(transform.position).x != -5)
            {
